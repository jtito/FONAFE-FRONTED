...
@Component({
  selector: 'app-matriz-anticorrupcion',
  templateUrl: './matriz-anticorrupcion.component.html',
  styleUrls: ['./matriz-anticorrupcion.component.scss']
})
export class MatrizAnticorrupcionComponent implements OnInit {
  ...
  listaVacia: BandejaMatrizRiesgo[];
    ...
    constructor(private matrizRiesgoService: MatrizAnticorrupcionService, private router: Router, private authLoginService: AuthLoginService) {
    ...
    listarBandejaMatrizRiesgo() {
        ...
        return this.matrizRiesgoService.listarBandejaMatrizRiesgo(this.idEmpresa, this.idPeriodo, this.idMatrizNivel).subscribe(
            resp => {
                console.log("RESP J "+JSON.stringify(resp.listaBandejaMatrizRiesgo));
    ...
    exportExcel(): void {
        import('xlsx').then(xlsx => {
            ...
            this.saveAsExcelFile(excelBuffer, 'MatrizRiesgosAnticorrupcion');
        });
    }
    ...
    eliminarMatrizRiesgo(idMatrizRiesgo: number): void {
        ...
        Swal.fire({
            title: '¿Está seguro de eliminar la Matriz de Riesgo Anticorrupción?',
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    html: 'Estamos eliminando la Matriz de Riesgo Anticorrupción',
                    allowOutsideClick: () => !Swal.isLoading(),
