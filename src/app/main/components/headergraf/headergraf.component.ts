...
import { HeaderGrafService } from './headergraf.service';
...
@Component({
  selector: 'app-headergraf',
  templateUrl: './headergraf.component.html',
  styleUrls: ['./headergraf.component.scss']
})

export class HeaderGrafComponent implements OnInit {
  ...
  @Output() tipomatrizChange = new EventEmitter<number>();
...
onChangeTipoMatriz(input) {
  this.idTipoMatriz = input.target.value;
  console.log("idTipoMatriz " +  this.idTipoMatriz);
 if(this.idPeriodo && this.idEmpresa){
    this.tipomatrizChange.emit(this.idTipoMatriz);
 } 
...
obtenerListaTipoMatriz() {
  ...
  this.headerService.obtenerListaTipoMatriz().subscribe(
      resp => {
          ...
          console.log('lista de Tipo MatrizR1 : ' + JSON.stringify(this.listaTiposMatrizRiesgo));
          if(this.listaTiposMatrizRiesgo[0]!=null){
            this.formularioHeadergraf.get('idTipoMatriz').setValue(this.listaTiposMatrizRiesgo[0].idParametro);
